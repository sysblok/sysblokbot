...
class TasksReportJob(BaseJob):
    ...
    def _execute(app_context: AppContext, send: Callable[[str], None]):
        ...
        paragraphs += (TasksReportJob._retrieve_cards_for_paragraph(list_cards, app_context))
    ...
    def _get_cards_without_members(cards):
        ...
        cards_without_members = sorted(cards_without_members, key=lambda card: card.due) 
    ...
    def _group_cards_by_date(cards, app_context):
        ...
        for card in cards:
            if card.due:
                ...
                result.append(deadline + card_text) # forms a line with the date and task 
            if not card.due:
                card_text_without_dates.append(TasksReportJob._format_card(card, app_context))
    ...
    def _retrieve_cards_for_paragraph(cards, app_context):
        ...
        for member in members:
            ...
            cards_text = TasksReportJob._group_cards_by_date(member_cards, app_context) 
        ...
        if cards_without_members:
            ...
            cards_without_members_text = TasksReportJob._group_cards_by_date(cards_without_members, app_context) 
