...
class HRStatusJob(BaseJob):
    ...
    def _execute(
        ...
        paragraphs = [
            load("hr_status_job__header", date=datetime.today().strftime("%d.%m"))
        ]
        ...
        for item in people_trial:
            ...
            person_paragraph = load(
                ...
                member_name=item.name,
                member_tg=item.telegram
                if item.telegram and item.telegram != "#N/A"
                else "",
                date_submitted=item.date_submitted.split()[0]
                if item.date_submitted
                else "???",
            ...
            if not item.telegram or item.telegram == "#N/A":
                person_paragraph += "\n" + load(
                    "hr_status_job__other_contacts", contacts=item.other_contacts
                )
    ...
    def _get_people(app_context: AppContext) -> List[HRPersonProcessed]:
        return [
            HRPersonProcessed(item)
            for item in app_context.sheets_client.fetch_hr_forms_processed()
        ]
