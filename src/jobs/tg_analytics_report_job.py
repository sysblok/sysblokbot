...
class TgAnalyticsReportJob(BaseJob):
    ...
    def _execute(app_context: AppContext, send: Callable[[str], None], called_from_handler=False):
        ...
        message = load(
            ...
            enabled_notifications_part=round(
                stats.enabled_notifications.part / stats.enabled_notifications.total * 100, 2
            ),
            recent_message_views=sum([
                message_stats.views for message_stats in stats.recent_message_interactions
            ]),
    ...
    def _get_followers_stats(stats) -> Tuple[int, int]:
        ...
        # filter dates
        data = list(filter(lambda x: stats.period.min_date <= x[0] <= stats.period.max_date, data))
