    ...
    def _execute(
        ...
        for member in members_with_telegram:
            ...
            if existing_user and existing_user.telegram_user_id:
                logger.debug(f"User already exists with telegram_user_id for {member.name}")
                skipped_count += 1
                continue
            
            # Check if User with this username already exists
            user_by_username = db_client.get_user_by_telegram_username(normalized_username)
            if user_by_username and user_by_username.telegram_user_id:
                ...
                if not user_by_username.team_member_id:
                    ...
                    linked_count += 1
                    logger.info(f"Linked existing User {user_by_username.id} to TeamMember {member.id}")
                skipped_count += 1
                continue
            ...
            if not result:
                logger.warning(f"Could not resolve username {telegram_username} for {member.name}")
                failed_count += 1
                continue
            ...
            if user_by_tg_id:
                ...
                if not user_by_tg_id.team_member_id:
                    ...
                    linked_count += 1
                    logger.info(f"Linked existing User {user_by_tg_id.id} to TeamMember {member.id}")
                
                # Update username if it changed
                if resolved_username and user_by_tg_id.telegram_username != resolved_username:
                ...
                created_count += 1
                logger.info(f"Created User {user.id} for {member.name} (tg_id={user_id})")
