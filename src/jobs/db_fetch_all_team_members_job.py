...
class DBFetchAllTeamMembersJob(BaseJob):
    ...
    async def _execute(
        ...
        await send(load("db_fetch_authors_sheet_job__success", num_authors=num_authors))

        num_curators = app_context.db_client.fetch_curators_sheet(
            app_context.sheets_client
        )
        logger.info(f"Fetched {num_curators} curators")
        await send(load("db_fetch_curators_sheet_job__success", num_curators=num_curators))
