...
async def _test_conversation_step(conversation: Conversation, message_to_send: str, expected_response_id: str, timeout: int):
...
async def _test_command_flow(report_state: PytestReport, conversation: Conversation, command_flow: list, timeout=120):
    ...
    try:
        ...
        for message_to_send, expected_response_id in command_flow:
            await _test_conversation_step(conversation, message_to_send, expected_response_id, timeout)
...
class TestTelegramBot:
    ...
    @pytest.mark.asyncio
    async def test_command_flows(self, conversation: Conversation, command_flow: List[tuple]):
    ...
    async def test_failing_command(self, conversation: Conversation, command: str):
        try:
            await _test_conversation_step(conversation, command, "should_not_matter", timeout=10)
        except TimeoutError:
            pass
